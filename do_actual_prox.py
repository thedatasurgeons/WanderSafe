...
def main():
    https_request = "https://cle.cit.api.here.com/2/search/proximity.json"
    payload={'app_id':'QJT4ednyb3ber0m5g75d',
             'app_code':'QplsyvIKvVKCdN3bF8MqPQ',
             'layer_ids':'1',
             'proximity':'%f,%f,%f' % (float(sys.argv[1]), float(sys.argv[2]), float(sys.argv[3]))}

    contentR = requests.get(https_request, params=payload)

    response = json.loads(contentR.text)
    print(response)
    for response_item in response["geometries"]:
        print(response_item["attributes"], response_item["distance"])
